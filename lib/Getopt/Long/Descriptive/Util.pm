package Getopt::Long::Descriptive::Util;
...
our @ISA       = qw(Exporter);
our @EXPORT_OK = qw(
                       detect_getopt_long_descriptive_script
               );

our %SPEC;

$SPEC{detect_getopt_long_descriptive_script} = {
    v => 1.1,
    summary => 'Detect whether a file is a Getopt::Long::Descriptive-based CLI script',
    description => <<'_',
...
* either: must be perl script (shebang line contains 'perl') and must contain
  something like `use Getopt::Long::Descriptive`;
...
sub detect_getopt_long_descriptive_script {
    my %args = @_;
  ...
  DETECT:
    {
        ...
        if ($str =~ /^\s*(use|require)\s+(Getopt::Long::Descriptive)(\s|;)/m) {
            $yesno = 1;
            $extrameta{'func.module'} = $2;
            last DETECT;
        }
        $reason = "Can't find any statement requiring Getopt::Long::Descriptive module";
    } # DETECT

    [200, "OK", $yesno, {"func.reason"=>$reason, %extrameta}];
}

#ABSTRACT: Utilities for Getopt::Long::Descriptive
